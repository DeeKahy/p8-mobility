...
export default function HomeScreen() {
  ...
  const {
    uri: capturedImageURI, // URI of the most recent picture taken via camera.tsx.
    mode: cameraMode, // Use this do decide what to do with the captured image.
  } = capturedImage;
  ...
  const handleNewMarkerFromPicture = async () => {
    ...
    router.push({
      pathname: "/camera",
      params: { mode: CameraMode.Addition },
    });
  };
  ...
  // Runs whenever capturedImage updates e.g. the cameraMode is changed or a new photo is taken using camera.tsx
  useEffect(() => {
    if (!capturedImageURI) return;
    if (cameraMode === CameraMode.Placement && resumableRef.current) {
    ...
    } else if (cameraMode === CameraMode.Addition) {
      ...
      withLoadingOverlay("Running blur detection...", async () => {
        await scoreAndToastBlur(capturedImageURI);
        log("Preparing photos for upload");
        return preparePhotosForUpload([capturedImageURI]);
      })
    ...
    <GestureHandlerRootView>
      <View style={styles.container}>
        ...
        <ResumableZoom
          ...
          onUpdate={(state) => {
            "worklet";
            if (cameraMode === CameraMode.Placement) {
          ...
          onLongPress={(event) => {
            if (cameraMode === CameraMode.Placement) {
              ...
              const imageDate = new Date().toISOString().split("T")[0];
              setPendingPhotos([{ uri: capturedImageURI, date: imageDate }]);
              setCapturedImage({
                uri: capturedImageURI,
                mode: CameraMode.None,
              });
        ...
        >
          <View style={styles.canvas}>
            ...
            {cameraMode === CameraMode.Placement || showTempMarker ? (
              <MarkerElement
                x={previewMarker.x}
                y={previewMarker.y}
        ...
        <Text style={styles.instructions}>
          {`${cameraMode === CameraMode.Placement ? `Tap and hold to ${selectedMarkerId ? "add the image" : "create marker"}` : "Tap on the floor plan to place a marker"}`}
        </Text>
